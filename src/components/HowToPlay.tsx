...
function HowToPlay() {
  const ItemList = ({ number, title, text }: { number: string, title: string, text: string }) => {
    return (
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 bg-white rounded-3xl sm:rounded-[2.5rem] p-8 sm:py-6">
        ...
        <div className="flex sm:flex-col items-center sm:items-start gap-8 sm:gap-6">
          ...
          <span className="hidden sm:block sm:text-xl text-purple_mountains_majesty tracking-wider leading-5">{ text }</span>
        </div>
  ...
  return (
    <div className="min-h-svh bg-mobile sm:bg-tablet bg-center bg-cover">
      <div className="flex flex-col gap-14 bg-gradient-to-b from-black_rock to-deep_koamaru px-6 sm:px-16 pt-6 pb-8">
