...
function HowToPlay() {
  const ItemList = ({ number, title, text }: { number: string, title: string, text: string }) => {
    return (
      <div className="lg:w-full flex flex-col sm:flex-row lg:justify-center sm:items-center gap-4 sm:gap-8 bg-white rounded-3xl sm:rounded-[2.5rem] p-8 lg:py-14">
        <h2 className="hidden sm:block lg:hidden sm:text-8xl text-blue_ribbon">{ number }</h2>
        <div className="flex sm:flex-col items-center sm:items-start lg:items-center gap-8 sm:gap-4 lg:gap-14">
          <h2 className="text-2xl sm:hidden lg:block lg:text-8xl text-blue_ribbon">{ number }</h2>
          <h3 className="text-2xl sm:text-4xl lg:text-5xl text-deep_koamaru tracking-wide uppercase">{ title }</h3>
          <span className="hidden sm:block lg:w-[16rem] text-xl lg:text-2xl lg:text-center text-purple_mountains_majesty tracking-wider leading-6">{ text }</span>
        </div>
  ...
  return (
    <div className="min-h-svh bg-mobile sm:bg-tablet bg-center bg-cover">
      <div className="flex flex-col gap-14 sm:gap-20 bg-gradient-to-b from-black_rock to-deep_koamaru px-6 sm:px-16 pt-6 pb-8 lg:px-28 lg:pt-8">
